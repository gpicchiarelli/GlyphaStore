...
auto Reactor::queue_owned_response(const ConnectionToken token, ResponseView response, OwnedValue value)
    ...
    if (current->tls || value.bytes.size() < reactor_detail::kMinimumScatterValueBytes || has_buffered_follow_up ||
    ...
    for (;;) {
        while (has_pending_output(*current)) {
            ...
            if (current->tls) {
                ...
                const auto* data = current->output.data() + current->output_offset;
                requested_size = current->output.size() - current->output_offset;
                const auto written = ::send(current->socket.descriptor(), data, requested_size, reactor_detail::send_flags());
                if (written > 0) {
                ...
                const auto written = ::sendmsg(current->socket.descriptor(), &message, reactor_detail::send_flags());
                if (written > 0) {
                    written_size = static_cast<std::size_t>(written);
                } else if (written < 0 && (errno == EAGAIN || errno == EWOULDBLOCK)) {
                    would_block = true;
