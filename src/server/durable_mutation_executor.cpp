    ...
    while (true) {
        ...
        std::uint64_t queue_wait_ns{};
        {
            ...
            lane.available.wait(lane_lock, [&] {
                return lane.stopping || lane.size != 0 || expire_remaining_.load(std::memory_order_acquire);
            });
            if (lane.size == 0) {
                return;
            }
        ...
        if (expired) {
            ...
            try {
                ...
                for (unsigned attempt = 0; attempt < 2; ++attempt) {
                    result = task->kind == DurableMutationKind::put
                                 ? detail::StoreAccess::put_durable(store_, worker_index, key, task->value,
                                                                    task->expire_at_ns)
                ...
                conflict_retry_committed = conflict_retried && result.committed() && !result.error;
        ...
        if (!expired) {
            const auto foreground_ns = service_ns > std::numeric_limits<std::uint64_t>::max() - queue_wait_ns
                                           ? std::numeric_limits<std::uint64_t>::max()
                                           : service_ns + queue_wait_ns;
        ...
        }
        {
            ...
            if (conflict_retried) {
                ++lane.conflict_retries;
            }
            if (conflict_retry_committed) {
                ++lane.conflict_retry_commits;
            }
