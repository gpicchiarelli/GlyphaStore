...
[[nodiscard]] inline auto as_string_view(const std::span<const std::byte> bytes) noexcept -> std::string_view {
    if (bytes.empty()) {
        return {};
    }
...
inline auto public_compaction_result(const std::size_t worker_index, const VacuumStats& stats) -> CompactionResult {
    return CompactionResult{
        .compacted = true,
        .worker_index = worker_index,
...
[[nodiscard]] inline auto sample_clock(const std::shared_ptr<const StoreClock>& clock) noexcept -> std::uint64_t {
