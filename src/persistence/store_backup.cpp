...
[[nodiscard]] auto copy_catalog_segments(int source_directory, int destination_directory,
    ...
    const auto workers =
        hooks_armed
            ? std::size_t{1}
            : std::max<std::size_t>(
                  1, std::min({max_parallelism, names.size(), kBackupSegmentCopyParallelism}));

    if (workers == 1) {
        SegmentCopyTotals totals{.workers_used = 1};
        for (const auto& name : names) {
            if (auto allowed = detail::invoke_filesystem_before(hooks, FilesystemOperation::copy_backup_segment);
                !allowed) {
    ...
    const auto worker_fn = [&] {
        for (;;) {
            ...
            auto copied =
                copy_named_file(source_directory, destination_directory, names[index].c_str());
            if (!copied) {
...
auto backup_durable_store_from_open_directory(DataDirectory& source, const Manifest& catalog_manifest,
                                              ...
                                              const bool verify_destination,
                                              const bool scan_source_records)
    ...
    report.destination_verify_ns = static_cast<std::uint64_t>(
