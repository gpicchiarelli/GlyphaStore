...
[[nodiscard]] auto recover_workers(recovery::RecoveryCatalog& catalog,
                                   recovery::RecoveryScanner& scanner,
    ...
    for (std::size_t worker_index = 0; worker_index < catalog.manifest.worker_count; ++worker_index) {
        ...
        auto worker = recovery::RecoveryIndexBuilder::build(
            worker_id, worker_index, catalog.manifest.worker_count, limits, std::move(*scan), recovered.stats);
        if (!worker) {
...
[[nodiscard]] auto assemble_recovery_state(recovery::RecoveryCatalog&& catalog,
                                           RecoveredWorkers&& recovered) -> DurableRecoveryState {
...
auto recover_durable_state(DataDirectory& directory, const std::uint64_t now_ns,
    ...
    auto catalog =
        recovery::load_recovery_catalog(directory, limits, compaction_intent, budget);
    if (!catalog) {
