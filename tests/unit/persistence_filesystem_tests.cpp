...
GLYPHA_TEST("positional IO fault matrix covers EINTR short IO and capacity errno classes") {
    ...
    // Repeated EINTR + single-byte short transfers still complete exact extents.
    {
        ...
        glyphastore::FileDescriptor file{
    ...
    const auto expect_write_failure = [](const int error_number, const glyphastore::ErrorCode expected) {
        TemporaryDirectory temporary;
    ...
    const auto expect_sync_failure = [](const int error_number, const glyphastore::ErrorCode expected) {
        TemporaryDirectory temporary;
        MatrixIo io{};
