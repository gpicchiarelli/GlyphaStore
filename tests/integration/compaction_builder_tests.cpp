...
GLYPHA_TEST("durable compaction rejected intent gate leaves the namespace untouched") {
    CompactionBuildDirectory temporary;
    auto directory = glyphastore::DataDirectory::open_and_lock(temporary.path());
    GLYPHA_REQUIRE(directory.has_value());
    auto fixture = create_build_fixture(*directory);
    bool gate_called{};
    const glyphastore::DurableCompactionIntentGate gate{
        .context = &gate_called,
        .acquire = [](void* opaque) -> glyphastore::Status {
            *static_cast<bool*>(opaque) = true;
            return glyphastore::fail(glyphastore::ErrorCode::sequence_conflict,
                                     "injected stale compaction authority");
        },
    };
    auto entries = fixture.index.entries();
    auto built = glyphastore::build_durable_worker_compaction(
        *directory, fixture.manifest, glyphastore::WorkerId{0}, std::move(entries), 100, {}, gate);
    GLYPHA_REQUIRE(gate_called);
    GLYPHA_REQUIRE(!built.succeeded());
    GLYPHA_REQUIRE(built.outcome == glyphastore::DurableCompactionBuildOutcome::not_started);
    GLYPHA_REQUIRE(built.error.has_value());
    GLYPHA_REQUIRE(built.error->code == glyphastore::ErrorCode::sequence_conflict);
    GLYPHA_REQUIRE(directory->read_manifest().value() == fixture.manifest);
    GLYPHA_REQUIRE(!directory->read_compaction_intent().has_value());
