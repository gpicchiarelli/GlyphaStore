...
distinct ports in one process; without it, TLS configuration makes `--port` TLS-only. Optional
Unix-domain sockets (`--unix-socket`, ADR 0029) use the same protocol and connection state; optional
`--unix-peercred` maps OS uid to `--authz-map` principals (`unix:uid=N`) and is not a TLS
replacement.
