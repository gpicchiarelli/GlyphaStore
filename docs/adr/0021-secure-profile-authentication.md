...
4. **OS user identity via Unix credentials only.** Rejected as sole model: IPv4 TCP is the primary
   transport; UDS/`getpeereid` is Phase 8 optional.
...
- Residual: compromised client key ⇒ full principal power until revoked (CRL/OCSP policy to be
  specified in Phase 3 reference; fail-closed when revocation configured but unavailable).
