...
(Phase 3), coarse capabilities (Phase 4), abuse controls (Phase 5), and Phase 6 auth audit + local
CRL fail-closed. Hostile public Internet exposure still wants an operator-configured CRL and
physical E3 honesty; live OCSP HTTP remains unsupported. Full multi-tenant isolation remains
Phase 8 (key-prefix scope is a first slice only — [ADR 0025](../adr/0025-key-prefix-tenant-scope.md)).
...
| Hostile multi-tenant share | Many principals, adversarial keys | **Unsupported** as a product; prefix scope + STATS gate + keyed Index seed ([ADR 0025](../adr/0025-key-prefix-tenant-scope.md)–[0027](../adr/0027-stats-isolation-prefix-principals.md)) improve posture; keyed Worker routing + data-dir isolation still open |
...
| Cross-tenant key access | optional `--authz-map` `prefix=` ([ADR 0025](../adr/0025-key-prefix-tenant-scope.md)); prefix `STATS` needs `admin` ([ADR 0027](../adr/0027-stats-isolation-prefix-principals.md)); Index mix seed keyed ([ADR 0026](../adr/0026-keyed-index-hash-seed.md)) | shared data dir (ADR 0028 deferred); no keyed Worker routing; hostile multi-tenant still unsupported |
...
| Anonymous remote mutate | mTLS + `--authz-map` / `--secure-profile` (default-deny; wire `PERMISSION_DENIED`) | Configure `--tls-crl` before hostile public Internet; multi-tenant Phase 8 |
| Local UDS anonymous mutate | optional `--unix-peercred` → `unix:uid=N` in `--authz-map` ([ADR 0029](../adr/0029-uds-peercred.md)); secure-profile requires peercred when UDS is enabled; socket mode `0600` | UDS is cleartext on-host; uid forgeable by root/same-uid; not a TLS replacement |
...
Hash-flood resistance is partial: Worker routing defaults to public FNV-1a and may use keyed
SipHash-2-4 with a Manifest-persisted seed ([ADR 0030](../adr/0030-keyed-worker-routing.md));
`--secure-profile` randomizes that seed unless `--worker-hash-seed` is pinned. Index placement uses
a separate process-lifetime keyed mix seed ([ADR 0026](../adr/0026-keyed-index-hash-seed.md)).
Hostile multi-tenant readiness remains incomplete without per-tenant data-dir isolation
([ADR 0028](../adr/0028-per-tenant-data-dir-deferred.md)).
...
No credentials should be stored in Store options or protocol fields until a credential lifecycle and redaction policy exists. TLS private keys and client certificates must never be written to logs or client `Error` messages. Backup/restore of a secure-profile host must copy cert/key/CA/CRL and `--authz-map` paths with the same permissions as the data directory (mode `0600` for keys); see [secure-profile-certs.md](../operations/secure-profile-certs.md).
...
The current safe deployment model is a single trusted application using the embedded API, or a daemon reachable only by mutually trusted local/private clients and protected by operating-system/network controls. Public Internet exposure and hostile multi-tenancy are not supported. Optional key-prefix scope on `--authz-map` can separate Store key namespaces for distinct mTLS principals; it does not make shared-daemon adversarial multi-tenancy a supported product.

Ordered implementation plan: [security roadmap](roadmap.md). Decisions: [ADR 0020](../adr/0020-tls-outer-transport.md), [ADR 0021](../adr/0021-secure-profile-authentication.md), [ADR 0022](../adr/0022-authorization-capabilities.md), [ADR 0025](../adr/0025-key-prefix-tenant-scope.md), [ADR 0026](../adr/0026-keyed-index-hash-seed.md), [ADR 0027](../adr/0027-stats-isolation-prefix-principals.md), [ADR 0028](../adr/0028-per-tenant-data-dir-deferred.md).
