...
| maintenance rate budget | Pre-intent physical-write pacing (`max_copy_bytes_per_sec`) plus a one-second adjacent-job/CPU (`max_cpu_ms_per_window`) controller window for **normal-mode Store compaction**. It is not a whole-transaction size cap. Distinct from Phase 5 connection/handshake limits and E3/E4 power-loss certification. Zero disables; pressure/emergency bypass. |
