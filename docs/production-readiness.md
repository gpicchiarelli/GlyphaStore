...
- [x] Authentication, authorization, transport security, rate limits, and audit requirements are specified.
  ...
  + `--tls-crl` + principal quotas (OVERLOADED) for cpp/python/go (+ perl/ruby/erlang when
  toolchains are installed). Residual: multi-tenant Phase 8 remainder (ADR 0028 deferred) and
  physical E3 honesty.
