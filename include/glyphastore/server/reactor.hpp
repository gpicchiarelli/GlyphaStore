...
class Reactor final {
  public:
    [[nodiscard]] static auto create(const ReactorConfig& config = {}) -> Result<std::unique_ptr<Reactor>>;
  ...
  private:
    ...
    Reactor(ReactorConfig config, TcpListener listener, Poller poller, Wakeup wakeup,
    ...
    static constexpr std::uint64_t kCompletionToken = kListenerToken - 1U;
