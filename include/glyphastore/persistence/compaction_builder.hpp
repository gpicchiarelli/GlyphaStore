...
// Optional runtime gate acquired after the read-only scan/layout phase and
// immediately before the durable intent. The gate must remain owned until the
// caller has either rolled the prepared transaction back or published its
// next manifest. A rejected gate performs no persistent namespace mutation.
...
build_durable_worker_compaction(DataDirectory& directory, const Manifest& current, WorkerId worker_id,
                                ...
                                const DurableResourceLimits& limits = {},
                                DurableCompactionIntentGate intent_gate = {}) -> DurableCompactionBuildResult;
